...
class AllocationService:
    ...
    def files(self, devices, files):
        ...
        files.sort(
            key=lambda x: (priority_map[x["priority"]], -x["size"]), reverse=True
        )
        ...
        for file in files:
            file_size_gb = self.bytes_to_gigabytes(
                file["size"]
    ...
    def blocks(self, devices, total_num_blocks, model_size_in_billion_parameters):
        ...
        # Todo: determine if it is even worth distributing the model
        ...
        for device in devices:
            ...
            print(
                f"Device: {device['device_name']}, RAM: {total_ram_gb:.2f} GB, Max Blocks: {device['max_blocks']}"
            )
