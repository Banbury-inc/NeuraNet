...
function download_file(files: string[], device: string[]): string {
  ...
  const fileHeader = `FILE_REQUEST:${files}:${file_size}:${username}:END_OF_HEADER`;
...
function main(files: string[], device: string[]): void {
  console.log('Downloading file...');
  const result = download_file(files, device);
  return;
}
